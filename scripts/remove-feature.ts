...
if (!removedFeatureName) {
  throw new Error('Provide feature flag name');
}

if (!featureState) {
  throw new Error('Provide feature state (on / off)');
}

if (featureState !== 'on' && featureState !== 'off') {
  throw new Error('Incorrect feature value stat (on / off)');
}
...
const getAttributeNodeByName = (jsxAttributes: JsxAttribute[], name: string) => jsxAttributes.find((node) => node
  .getNameNode()
  .getText() === name);
